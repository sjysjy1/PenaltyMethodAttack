...
ddn_L1attack_ALMA=torch.load('../result/mnist/ddn_attack_ALMA_pnorml1_iternum1000_initlr0.5.pt')
pert_list7=np.asarray(ddn_L1attack_ALMA['list_pert'].cpu())
...
y7=[len(pert_list7[np.asarray(pert_list7<item) & np.asarray(ddn_L1attack_ALMA['list_success_fail'].cpu())])/len(pert_list7) for item in x ]
