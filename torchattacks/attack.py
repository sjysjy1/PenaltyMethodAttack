...
class Attack(object):
    ...
    def __call__(self, inputs, labels=None, *args, **kwargs):
        ...
        if self._normalization_applied is True:
            ...
            #adv_inputs,delta,num,success,loss, predict_label = self.forward(inputs, labels, *args, **kwargs) #sun, for Zouten
            adv_inputs,num, predict_label = self.forward(inputs, labels, *args, **kwargs) #sun
            # adv_inputs = self.to_type(adv_inputs, self.return_type)

        self._recover_model_mode(given_training)

        return adv_inputs,num,predict_label #sunjiyuan
